...
- ✅ **文件树集成**：数据库图标、点击打开视图
- 🔨 **重构中**：改为 YAML frontmatter 驱动，笔记即数据
...
| 任务 | 说明 | 工作量 |
|------|------|--------|
| YAML 解析器 | 读取笔记 frontmatter | 0.5天 |
| 数据源重构 | 从 JSON rows → 扫描笔记 | 1天 |
| 双向同步 | 编辑表格 → 写回 YAML | 1天 |
| 数据库定义 | 只存列结构，不存数据 | 0.5天 |
| 刷新机制 | 打开时/文件变化时更新 | 0.5天 |
